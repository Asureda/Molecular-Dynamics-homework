...
def binning(array,magnitude):
     ...
     try:
          ...
          plt.xlabel('m')
          plt.ylabel('$\sigma_m$')
          plt.plot(vec_m,np.sqrt(32768-1)*vec_s2,'x',color='black',markersize=5,label='data'+' '+magnitude)
          plt.plot(xx,np.sqrt(32768-1)*fitting(xx,af,bf,tf),'-.',color='black',linewidth=0.5, label='fit')
          plt.legend(loc='best')
          plt.show()
...
average3,sigma3,b3,tt3=binning(energy,'energy')
